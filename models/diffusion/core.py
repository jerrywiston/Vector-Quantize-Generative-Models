...
class DDPMSampler(nn.Module):
    ...
    @torch.no_grad()
    def sample_one_step(self, x_t, time_step: int, c):
        """
    ...
    @torch.no_grad()
    def forward(self, x_t, c=None, only_return_x_0: bool = True, interval: int = 1, **kwargs):
        """
        Parameters:
...
class DDIMSampler(nn.Module):
    ...
    @torch.no_grad()
    def sample_one_step(self, x_t, time_step: int, c, prev_time_step: int, eta: float):
        t = torch.full((x_t.shape[0],), time_step, device=x_t.device, dtype=torch.long)
    ...
    @torch.no_grad()
    def forward(self, x_t, c=None, steps: int = 1, method="linear", eta=0.0,
