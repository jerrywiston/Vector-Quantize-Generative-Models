# Modified by https://github.com/alexanderfast/roguelike-dungeon-generator
#!/bin/python3
import random
import itertools
import sys
...
def generate(cellsX, cellsY, cellSize=5):
    ...
    # Plot Corridor
    for xy in itertools.chain.from_iterable(corridors):
        tiles[xy] = 0#"*"
    ...
    for y in range(tilesY):
        tile_row = []
        for x in range(tilesX):
            #print(str(tiles[(x, y)]).zfill(3), end=" ")
            tile_row.append(tiles[(x, y)])
        #print("\n")
        tiles_list.append(tile_row)
